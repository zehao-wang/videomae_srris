...
for file_path in paths:
    ...
    feat_list, _ = extractor.extract(str(file_path), no_label=True, use_clip_shift=True)
    out_file = f'{args.out_root}/{video_id}/{cam_type}.npy'
    print(f'\033[1;32m [INFO]\033[0m Output to {out_file}')
    np.save(out_file, {"hidden": feat_list, "labels": event_seq, "id2label": extractor.id2label})
